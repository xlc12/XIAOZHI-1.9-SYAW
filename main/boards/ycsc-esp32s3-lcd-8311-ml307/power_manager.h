...
private:
    ...
    static constexpr struct {
        uint16_t adc;
        uint8_t level;
    } BATTERY_LEVELS[] = {{2100, 0}, {2370, 100}};
    ...
    void ReadBatteryAdcData() {
        ...
        ESP_LOGI("PowerManager", "ADC值: %d 平均值: %ld 电量: %u%% 充电状态: %s", 
                 adc_value, average_adc, battery_level_, is_charging_ ? "充电中" : "未充电");
    }
...
public:
    PowerManager(gpio_num_t charging_pin, adc_unit_t adc_unit = ADC_UNIT_2,
        ...
        ESP_ERROR_CHECK(esp_timer_start_periodic(timer_handle_, 4000000));  // 10秒
